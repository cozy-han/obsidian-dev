...
### 관제 종료 조건
- 마지막 이력 생성 후 **GP_END_TIME(기본 1분)** 경과 시 자동 종료
- 배치([app-batch]])에서 처리

### 종료 시 계산
- **총 비행시간**: 첫 이력 ~ 마지막 이력 시간 차이 (밀리초)
- **총 비행거리**: Haversine 공식으로 인접 좌표 간 거리 누적 (5000건 청크 단위)
- **평균 속도**: 총 거리 / 총 시간

### 비정상 상황 캐싱
- Redis에 관제 ID별 비정상 상황 데이터 캐싱
- 관제 종료 시 Redis 데이터 삭제

### module-psty vs app-psty 구분

| | module/psty | app/psty |
|--|------------|---------|
| **역할** | 엔티티/리포지토리 (데이터) | 메시지 처리 엔진 (로직) |
| **내용** | JPA Entity, Repository | RabbitMQ Consumer, SocketCommand |
| **성격** | 도메인 모델 | 실행 가능 앱 (포트 8400) |
